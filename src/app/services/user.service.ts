...
export class UserService {
  ...
  async getUsuarioPorCorreo(mail: string)
  {
    ...
    for (const collectionName of collections) {
      const userQuery = query(collection(this.firestore, collectionName), where('mail', '==', mail));
      const querySnapshot = await getDocs(userQuery);

      if (!querySnapshot.empty) {
        const userDoc = querySnapshot.docs[0];
        const userData = await userDoc.data() as Usuario;
        this.personaLogeada = userData;
        break;
      }
    }
    return this.personaLogeada as Usuario;
  }
  ...
  usuarioActivo(): boolean {
    return this.personaLogeada.activo;
  }
